...
<div class="card">
    <div class="card-body">
            @include ('alert')
            @if ($old_sch && Session::get('success')=='')
            <div class="alert alert-danger alert-block">
                <button type="button" class="close" data-dismiss="alert">×</button>	
                    <strong>{{__('Already have schedule on')}} {{$old_sch->date}}. {{__('Please choose different date')}}.</strong>
            </div>
            @endif
            <h4 class="card-title">{{__('Create new')}} {{strtoupper($single_range)}} {{__('Schedule')}}</h4>           
            ...
            <div class="row form-row">										
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                        <form action="{{url('saveschedule')}}" method="POST" id="schedule-form">
                            @csrf
                            <input type="hidden" name="single_range" value="{{$single_range}}" />                     
                            <div class="form-group">
                                <label>{{__('Date Range')}}<span class="text-danger">*</span></label>
                                <div class="option-daterange btn btn-white mb-3 w-100">
                                    <i class="far fa-calendar-alt mr-2"></i>
                                    <span></span>
                                    <i class="fas fa-chevron-down ml-2"></i>                                                    
                                </div>
                                <input type="hidden" name="range_start" id="range_start_date" value="{{$st_date}}"/>
                                <input type="hidden" name="range_end" id="range_end_date" value="{{$ed_date}}"/>
                            </div>                                
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>{{__('Timing Schedules')}} (Monday ~ Sunday)<span class="text-danger">*</span></label>
                                <select class="form-control" name="slots_val_optoins"  id="slots_val_optoins">
                                    <option value="default">{{__('Use default timing schedule')}}</option>
                                    <option value="custom">{{__('Custom timing schedule')}}</option>
                                </select>
                            </div>         
                            <div class="form-group">
                                ...
                                <select class="form-control" name="holiday_val_optoins"  id="holiday_val_optoins">
                                    <option value="default">{{__('Use Holiday Settings')}}</option>
                                    <option value="ignore" id="ignore_setting_option">{{__('Ignore Holiday')}}</option>
                                </select>
                            </div>                                 
                        </form>                                                                                                                                      
                        </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-hover table-center mb-0  table-bordered">
                                ...
                                <tbody id="slot-custom-body">

                                </tbody>
                                <tbody id="slot-default-body">  
                                    <?php
                                    ...
                                    @foreach ($week_array as $one_weekday)
                                        ...
                                        @if($hd) 
                                        <tr>
                                            <td class="bg-success-light">{{strtoupper($one_weekday)}}</td>
                                            <td colspan="4" class="text-success text-center"><i class="fa fa-gifts"></i> {{__('Holiday')}}</td>
                                        </tr>
                                        @else
                                        <?php $old_weekday = '';?>
                                        @foreach ($default_timings as $one)
                                            ...
                                            <tr>
                                                <td class="bg-success-light">{{$one->weekday != $old_weekday?strtoupper($one_weekday):''}}</td>
                                                <td>{{\Carbon\Carbon::parse($one->start)->format('g:i A')}}</td>
                                                <td>{{\Carbon\Carbon::parse($one->end)->format('g:i A')}}</td>
                                                <td>{{$one->duration}} Minutes</td>
                                                <td>
                                                    <span class="badge badge-pill bg-{{$one->type?'primary':'info'}} text-white">{{$one->type?'Onsite':'Online'}} {{__('Meeting')}}
                                                </td>
                                                <?php $old_weekday = $one_weekday;?>
                                            </tr>
...
@section('page-js')
    ...
    <script>
        $(document).ready(function(){
            ...
            form_submit_data['start_date'] = $('#range_start_date').val();
            form_submit_data['end_date'] = $('#range_end_date').val();
            ...
            $('#add_slot_popsave').click(function(){
                ...
                var slot_index = Math.floor((Math.random() * 100) + 1);
                ...
                tr+='<tr>'; 
                tr+='<td></td>';
                tr+='<td>' + start_time + '</td>';tr+='<td>' + end_time  + '</td>';tr+='<td>' + dur + ' Minutes </td>'; tr+='<td>' + (type=='onsite'?'Onsite Meeting':'Online Meeting') + '</td>';
            ...
            if($('.option-daterange').length > 0) {
                var start = moment($('input[name=range_start]').val());
                var end = moment($('input[name=range_end]').val());
                function booking_range(start, end) {
                    $('input[name=range_start]').val(start.format('YYYY-MM-DD'));                    
                    $('input[name=range_end]').val(end.format('YYYY-MM-DD'));
                    $('.option-daterange span').html(start.format('YYYY-MM-DD') + ' - ' + end.format('YYYY-MM-DD'));
                }
