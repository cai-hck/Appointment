...
@section('page-css')
	...
	<style>
	 ...
	 #form {      
	  ...
	  .next, .prev, .next-mission-arrow, .next-section-arrow, .next-date-arrow ,.submit-book-btn,.personal-next ,.verify-next{
		padding: 15px 25px;
		font-size: 18px;
		font-weight: bold;
	  }
	...
	<div id="smartwizard" style="min-height:300px">
		...
		<ul class="nav">
			...
			<a class="nav-link" href="#step-1">
				<strong>{{__('Step 1')}}</strong> <br>{{__('Choose a Mission')}}
			</a>
			</li>
			<li class="nav-item">
			<a class="nav-link" href="#step-2" data-repo="section">
				<strong>{{__('Step 2')}}</strong> <br>{{__('Choose a Section')}}
			</a>
			</li>
			<li class="nav-item">
			<a class="nav-link" href="#step-3" data-repo="date">
				<strong>{{__('Step 3')}}</strong> <br>{{__('Choose a Date')}}
			</a>
			</li>
			<li class="nav-item">
			<a class="nav-link " href="#step-4">
				<strong>{{__('Step 4')}}</strong> <br>{{__('Confirm Document')}}
			</a>
			</li>
			<li class="nav-item">
			<a class="nav-link " href="#step-5">
				<strong>{{__('Step 5')}}</strong> <br>{{__('Enter Information')}}
			</a>
			</li>			
			<li class="nav-item">
			<a class="nav-link " href="#step-6">
				<strong>{{__('Step 6')}}</strong> <br>{{__('Confirm SMS')}}
			</a>
			</li>			
		</ul>
							...
							<div class="col-md-6 col-lg-4 col-sm-12">						
								...
								<div class="blog grid-blog">
									<div class="blog-image">
										<a href="javascript:void(0)"><img class="img-fluid" src="{{ asset($one->cover_image) }}" alt="Post Image"></a>
									</div>
									<div class="blog-content">										
										...
										<p class="mb-0">
										@if ($one->setting)
										<a data-mid="{{$one->id}}" href="{{url('/m/'.$one->setting->slug)}}" class="btn btn-block btn-sm text-white btn-success next-mission-arrow">
											<strong> {{__('Next')}}  <i class="fa fa-chevron-right"></i> </strong> </a>							
										</p>
										@endif
									</div>
								</div>
			...
			<div id="step-2" class="tab-pane" role="tabpanel" aria-labelledby="step-2">
				...
				<div class="form-group">	
					<div class="col-md-12 col-sm-12 d-flex" style="justify-content:space-between">
						...
						<a href="javascript:void(0);" class="btn btn-sm btn-success  next">{{__('Next')}} <i class="fa fa-chevron-right"></i> </a>
					</div>
				</div>				
			</div>
 ...
 $(document).ready(function(){
	...
	function option_file_board(doc_list, type) {
		...
		for (var i=0;i<doc_list.length;i++) {
			...
			<?php if (app()->getLocale() == 'en') { ?>
				html+='<label>'+ doc_list[i]['en'] + ' {{__("File")}} ({{__("Optional") }})'+'</label>';
			<?php } else { ?>
				html+='<label>' + doc_list[i]['ar'] + ' {{__("File")}} ({{__("Optional") }})'+'</label>';
			<?php } ?>
			html+='<input class="form-control" type="file" name="file[]" style="padding-top:15px;">';
			...
			if (repo == 'section') {
				return new Promise((resolve, reject) => {
					$.ajax({
						method  : "GET",
						url: ajaxURL,
						data: { _token:"{{csrf_token()}}", mid: $('#mission_id').val() ,sid:0,curdate:'' },
						beforeSend: function( xhr ) {
							// Show the loader
							$('#smartwizard').smartWizard("loader", "show");
						}
						...
						var html=""
						html+='<select class="form-control" name="section_sel" id="sel-section">';
						html+='<option value="">' + "{{__('Choose a Section')}}" +'</option>';
						for (var i=0;i<res.length;i++) {
							@if (app()->getLocale()=='en')
							html+='<option value="' + res[i]['id'] + '">' + res[i]['en_name'] + '  </option>';
							@else
							html+='<option value="' + res[i]['id'] + '">' + res[i]['ar_name'] + '  </option>';
							@endif
						}				
						html+='</select>';

						html+='<div class="col-md-12 mt-3 mb-3">';
						html+='<div class="form-group text-right" id="action-part" hidden>';					
						html+='<label class="ml-3 mr-3"><input type="checkbox" id="agree_term"/><strong>&nbsp;&nbsp;'+ "{{ __('I agree, Understand') }}" +'</strong> </label>';
						html+='<a href="javascript:void(0)" class="btn btn-sm btn-success next-section-arrow"><strong>' + "{{__('Next') }}"+' <i class="fa fa-chevron-right" ></i></strong></a>';
						html+='</div>';
						html+='<div class="form-group" id="section-text-content" >';
